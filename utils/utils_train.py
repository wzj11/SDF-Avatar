...
def _ssim(img1, img2, window, window_size, channel, size_average=True, mask=None):
    ...
    # new
    sigma1_sq = F.relu(sigma1_sq) # 或者 .clamp(min=0)
    ...
    if mask is not None:
        ...
        # print(mask.shape)
        # exit()
        return (ssim_map * mask).sum() / (mask.sum() * ssim_map.shape[-3]  + 1e-6)
...
def l1_loss(network_output, gt, type=None):
    if type == 'color':
        ...
        return torch.abs(lin_pred - lin_gt).mean()
    else:
...
def mesh2smplx(mname, mesh=None, return_=False, device='cuda:0', output_dir='output_ners'):
    from utils3d.torch import quaternion_to_matrix

    trans = np.load(f'{output_dir}/{mname}/params/trans.npz')
    kt = np.load(f'{output_dir}/{mname}/params/kt.npz')
    k, t = list(kt.values())
...
def loss_recon(gt, pred, lambda_ssim=None, lambda_norm=None, mask=None):
    if lambda_ssim is not None:
        if lambda_norm is not None:
            return lambda_norm * l1_loss(gt, pred) + lambda_ssim * (1 - ssim(gt, pred, mask=mask))
        else:
            return 1 * l1_loss(gt, pred) + lambda_ssim * (1 - ssim(gt, pred, mask=mask))
    else:
        if lambda_norm is not None:
            return lambda_norm * l1_loss(gt, pred) + 0.2 * (1 - ssim(gt, pred, mask=mask))
        else:
            return 1 * l1_loss(gt, pred) + 0.2 * (1 - ssim(gt, pred, mask=mask))
